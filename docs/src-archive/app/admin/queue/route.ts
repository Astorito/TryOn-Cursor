...
export async function GET(request: NextRequest) {
  try {
    ...
    // Import bull-board dynamically because it's an optional dev dependency
    let bullBoardRouter: any = null;
    try {
      // @ts-ignore - optional dependency; may not be installed in deployment
      const bb = await import('@bull-board/api')
      // @ts-ignore - optional dependency
      const bbAdapter = await import('@bull-board/api/bullMQAdapter')
      const { createBullBoard } = bb as any
      const { BullMQAdapter } = bbAdapter as any
      const board = createBullBoard([
        new BullMQAdapter(generationQueue.getQueueInstance())
      ])
      bullBoardRouter = board.router
    } catch (impErr) {
      console.error('[BullBoard] Optional dependency not installed or failed to import:', impErr)
      return NextResponse.json({ success: false, error: 'Bull Board is not available on this deployment' }, { status: 501 })
    }
